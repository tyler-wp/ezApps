                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="mail-container">
                                    ...
                                    <div class="mail-text">
                                        <p><?php $appResponses = "SELECT * FROM applicant_responses WHERE aid = ?";
                                            $appResponses = $pdo->prepare($appResponses);
                                            $appResponses->execute([$_SESSION['app_id']]);
                                            $apprDB = $appResponses->fetchAll(PDO::FETCH_ASSOC);
                                                                            
                                            foreach ($apprDB as $response) {
                                                $appQuestions = "SELECT * FROM applicant_inputs WHERE id = ?";
                                                $appQuestions = $pdo->prepare($appQuestions);
                                                $appQuestions->execute([$response['qid']]);
                                                $appqrDB = $appQuestions->fetch();
                                                
                                                echo '<strong>' . $appqrDB['question'] . '</strong>: ' . $response['response'] . '<br />';
                                            }; ?></p>
                                    </div>
